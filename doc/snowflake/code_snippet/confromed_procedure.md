...
DECLARE
    ...
    RECORD_COUNT              NUMBER DEFAULT 0; ✏️
...
    <YOUR LOGIC>;
    ...
    -- Checking the record count in the target table  ✏️
        SELECT COUNT(*) INTO :RECORD_COUNT
        FROM IDENTIFIER(:TARGET_TABLE)
        WHERE CAST(CREATEDON AS DATE) = CURRENT_DATE();

    -- Check if the Record Count is Greater Than 0 and Send Success Email Notification  ✏️
